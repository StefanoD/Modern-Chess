...
int search_root(uint8_t depth, int alpha, int beta) {
	...
	uint8_t mcount = movegen(movelist, bestmove);
...
int info_pv(int val) {
	...
	uint32_t time = gettime() - sd.starttime;
...
*  nps during longer searches. Node count is converted from uint64_t to unsigned   *
